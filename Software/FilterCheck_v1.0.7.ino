...
// Global Variables:
  ...
  int i = 0, j = 0, k = 3, counter = 0, pressure_counter = 0, led_counter = 0;
  bool Loop = true;
  unsigned long previousTime = 0;
  uint32_t start_delay = 4 * 10000L; // 40 Seconds
...
{   
  ...
  // 40 Second Delay to allow the flow meter time to boot up
    for (uint32_t tStart = millis(); ((millis() - tStart) <= start_delay);  ) 
    {
     ...
     {
      USBSERIAL.println("System boot up in 20 seconds!");
     }

     else if (led_counter == 30) 
     {
      USBSERIAL.println("System boot up in 10 seconds!");
     }
...
void loop() 
{
  ...
  // Blink LED's & Zero the Low-Pressure Sensor if there no filter is on
    if (pressure >= -.3 && (flow >= 30 && flow <= 120))
    {
      digitalWrite(GREEN_LED, LOW);  // Turn off Green LED
      digitalWrite(BLUE_LED, HIGH);  // Turn off Green LED
      delay(350);
      digitalWrite(BLUE_LED, LOW);  // Turn on Green LED (to make it blink)
    }

  // Blink LED's if there is a filter on
    if (pressure < -.4  && (flow >= 70 && flow <= 86.19))
    {
  ...
  // Flow Verification Loop:
    while ((flow >= 83.81 && flow <= 86.19) && k == 3)
    {
      ...
      flow = Read_Sensor("Flow");  // Read the Flow from the TSI 5310-2 FlowMeter
      pressure = Read_Sensor("Pressure");  // Read the Pressure from the TSI 5310-2 flow meter
      digitalWrite(GREEN_LED, HIGH);  // Turn on Green LED
      pressure_counter += 1;
      if (pressure >= -.3 && pressure_counter == 3) 
      {
        FLOWMETER.print("LPZ\r\n");  // Zeroes Low Pressure Sensor
        if (DEBUG) {USBSERIAL.println("Low Pressure Sensor Zeroed!");}
        pressure_counter = 0;
      }
      else if (pressure_counter > 3 )
      {
        pressure_counter = 0;
      }
      delay(350);
